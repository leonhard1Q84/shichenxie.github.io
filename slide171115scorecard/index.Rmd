...
# print(xtable::xtable(dt[8,c(3:1)]), include.rownames = FALSE, type = "html", html.table.attributes = "border=0")
...
```{r results = "asis", echo=FALSE}
# print(xtable::xtable(setnames(train_score[8,], sub("_points$","",names(train_score)))[,lapply(.SD,as.integer)]), include.rownames = FALSE, type = "html", html.table.attributes = "border=0")
knitr::kable(setnames(train_score[8,], sub("_points$","",names(train_score)))[,lapply(.SD,as.integer)], format = "html")
```
...
- **自定义启动环境**<sup>*</sup>: 
  ...
  - 用户主目录路径可通过`Sys.getenv("HOME")`获取或者首次打开R时通过`getwd()`获得。
...
.pull-left[
以`scorecard`包([Github](https://github.com/ShichenXie/scorecard))为例: 

1. 安装包，从CRAN中安装`install.packages("scorecard")`

1. 加载包, `library(scorecard)`
...
- 目前[CRAN - Packages](https://mirrors.tuna.tsinghua.edu.cn/CRAN/web/packages/index.html)上有11815个包，如何找到有用的R包: 
  ...
  - [R语言常用包介绍](https://support.rstudio.com/hc/en-us/articles/201057987-Quick-list-of-useful-R-packages)
...
dt <- setDT(germancredit)[
  , creditability := ifelse(creditability=="bad",1,0)]
# 分箱
bins <- woebin(dt, y="creditability", print_step=0L)
bins$age.in.years
```
...
- 信息值(Information Value)是衡量一个二元变量y和一个名义变量x之间的关联性的指标。在评分卡模型开发中，通过信息值大小判断候选自变量的预测力。信息值定义如下:
...
# print(xtable::xtable(iv_df), include.rownames = FALSE, type = "html", html.table.attributes = "border=0")
knitr::kable(iv_df, format = "html")
...
- `scorecard::woebin`分箱函数：模型候选变量包括连续变量、类别变量、有序类别变量三类，分别对应R语言中的numeric、character、factor数据类型。
...
--
    ...
    - 最后，通过决策树实现有序类别变量的最优分箱，其算法过程见下一页
      ...
      - 分箱停止条件是信息增益率`stop_limit`与最大分箱数 `max_bin_num`(不包括缺失值)，默认值分别为0.1与5，即信息增益率小于10%或者非缺失值分箱数大于5后停止进一步分箱。

.footnote[[*] 极端值：均值上下3倍四分位距以外的值，其中四分位距指上下四分位数之差。]
...
.pull-right[
`scorecard::woebin_plot`给出了分箱信息图：
  - 坏客户率或者WOE值的趋势线，最多不超过一个拐点，最好是单调的
  - 每个分箱的样本数量占比需大于5%
```{r}
...
woebin_plot(bin_adj$age.in.years)
```
]

```{r}
# 调整后分箱信息
bin_adj$age.in.years[,.(bin,woe)]
```
...
# WOE转换
dt_woe <- woebin_ply(
  dt, bin_adj, print_step=0L)

cbind(dt[1:5,.(age.in.years)],
      dt_woe[1:5,.(age.in.years_woe)])
```
]
...
```{r}
dat2 <- split_df(dt, y="creditability", ratio=0.6, seed=40)
train <- dat2$train; test <- dat2$test
```
...
```{r comment='#', message=FALSE, warning=FALSE}
dt_sel<-var_filter(dt_woe,y="creditability") #粗筛后数据集
var_sel <- sub("^(.+)_woe$","\\1",names(dt_sel))
```

--

```{r comment='#', message=FALSE, warning=FALSE}
dt_sel2<-var_filter(dt_woe,y="creditability",return_rm_reason=TRUE)
# dt_sel2$dt # 返回粗筛后数据集 # dt_sel2$rm # 返回删除变量的原因
```

```{r results = "asis", echo=FALSE, message=FALSE}
# 删除变量的原因
# print(xtable::xtable(dt_sel2$rm), include.rownames = FALSE, type = "html", html.table.attributes = "border=0")
...
逻辑回归`(logistic regression)`在信用评分卡开发中起到核心作用。逻辑回归通过`sigmoid`函数 $y=1/(1+e^{-z})$ 将线性回归模型 $z=\boldsymbol{w}^T\boldsymbol{x}+b$ 产生的预测值 $z$ 转换为一个接近0或1的 $y$ 值，变化后得到：
$$\ln\frac{y}{1-y}=z=\boldsymbol{w}^T\boldsymbol{x}+b$$
其中， $y/(1-y)$ 为比率(odds)，即违约概率与正常概率的比值。 $\ln\frac{y}{1-y}$ 为logit函数，即比率的自然对数。因此，逻辑回归实际上是用线性回归模型的预测结果逼近真实标签的logit。
...
```{r echo=FALSE}
train_woe <- woebin_ply(train[,var_sel,with=F], bin_adj, print_step=0L)
test_woe <- woebin_ply(test[,var_sel,with=F], bin_adj, print_step=0L)
...
```{r}
perf_train <- perf_eva(train$creditability, train_pred)
perf_train$pic
```
...
其中，违约与正常的比率定义为一个客户被划分为违约的估计概率与被划分为正常的估计概率之间的比率 $odds=p/(1-p)$, `A`与`B`是常数。
...
- `scorecard::scorecard`函数默认设置`points0=600, odds=1/19, pdo=50`

```{r warning=FALSE, message=FALSE}
# 创建评分卡刻度
card <- scorecard(bins, m2)

rbindlist(card, fill = TRUE)[1:4,1:4]
```
...
```{r warning=FALSE, message=FALSE}
# 评分转换
train_score <- scorecard_ply(train, card, print_step=0L)
test_score <- scorecard_ply(test, card, print_step=0L)
...
稳定性指数`(population stability index, PSI)`是计算实际和逾期的分值分布之间差异的一个衡量指标,  $PSI=\sum_i{(A_i-E_i)\ln(A_i/E_i)}$ 。其计算过程见下表: 
...
```{r results = "asis", echo=FALSE, message=FALSE}
# print(xtable::xtable(psi_cal), include.rownames = FALSE, type = "html", html.table.attributes = "border=0")
...
# print(xtable::xtable(psi_rule), include.rownames = FALSE, type = "html", html.table.attributes = "border=0")
