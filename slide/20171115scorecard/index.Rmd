...
&=& (A-Bb) - Bw_1x_1 - Bw_2x_2 \cdots - Bw_nx_n
\end{eqnarray}$$
其中， $x_1\cdots x_n$ 为最终进入模型的自变量且已经转换为WOE值,  $w_i$ 为逻辑回归的变量系数,  $b$ 为逻辑回归的截距, $A, B$ 为上页求得的刻度因子。 $Bw_ix_i$ 为变量 $x_i$ 对应的评分， $(A-Bb)$ 为基础分(也可将基础分值平均分配给各个变量)。
--

- `scorecard::scorecard`函数创建评分卡刻度，其默认设置`points0=600, odds=1/19, pdo=50`
```{r card, warning=FALSE, message=FALSE}
card <- scorecard(bins_adj, m2) # 创建评分卡刻度
rbindlist(card, fill=TRUE)[1:2,1:4] 
```
- `scorecard::scorecard_ply`函数可将原始数据集转换为信用评分
```{r score, warning=FALSE, message=FALSE}
train_score <- scorecard_ply(train, card, print_step=0) # 评分转换
train_score[1:2,]
```
