...
.pull-left[
```{r gradient_descent, echo=FALSE, fig.height=2, fig.width=3}
library(ggplot2)
...
$$\min{J(\boldsymbol{w},b)}+\lambda(\alpha||\boldsymbol{w}||_1+(1-\alpha)||\boldsymbol{w}||_2^2)$$
其中，正则化参数 $\lambda>0$ ; $||\boldsymbol{w}||_1$ 与 $||\boldsymbol{w}||_2^2$ 分别为L1与L2范数正则化，也分别称为LASSO(Least Absolute Shrinkage and Selection Operator)与"岭回归"(ridge regression)。L1与L2范数正则化都有助于降低过拟合风险，但前者更易于获得稀疏解，即求得的 $w$ 会有更少的非零分量。使用lasso筛选变量的案例如下：
...
family="binomial", nfolds=0, alpha=1, lambda_search=TRUE)
# summary(fit)

# variable importance
varimp <- data.table(h2o.varimp(fit))[names!=""][!is.na(coefficients) & coefficients > 0 & sign == "POS"]
var_sel2 <- c(varimp$names, "creditability")
```
